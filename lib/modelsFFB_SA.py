...
class PolyRegression(nn.Module):
    def __init__(self,
        ...
        # Self-Attention (moved after flip_block)
        self.attention = SelfAttention(embed_size=num_outputs, heads=attention_heads)

        self.channel_adapter = nn.Conv2d(256, 3, kernel_size=1)
        self.softadapt = LossWeightedSoftAdapt(beta=0.1)
        self.adapt_weight = torch.tensor([300,1,1,1,300,1])
        self.loss_history = []
    ...
    def forward(self, x, epoch=None, **kwargs):
        ...
        # Apply Self-Attention after FeatureFlipBlock and PAPFN
    ...
    def loss(self,
        ...
        if self.share_top_y:
            ...
            pred_lowers[...] = pred_lowers[:, 0].reshape(-1, 1).repeat(1, pred.shape[1])
        ...
        # Tập hợp các thành phần mất mát
        losses = [poly_loss, lower_loss, upper_loss, cls_loss, line_iou_loss, conf_loss]

                # Khởi tạo danh sách để lưu trữ các trọng số
        ...
        # Cập nhật giá trị trọng số vào từng danh sách
        self.weight_history['conf'].append(self.adapt_weight[5].item())
        self.weight_history['lower'].append(self.adapt_weight[1].item())
        self.weight_history['upper'].append(self.adapt_weight[2].item())
        self.weight_history['cls'].append(self.adapt_weight[3].item())
        self.weight_history['poly'].append(self.adapt_weight[0].item())
        self.weight_history['line_iou'].append(self.adapt_weight[4].item()) 

        # Đảm bảo mỗi danh sách chỉ giữ lại 10 giá trị gần nhất
        # for key in self.weight_history:
        #     self.weight_history[key] = self.weight_history[key][-10:]

        lenw = 0;
        
        # Debug: Kiểm tra danh sách trọng số
        for key, weights in self.weight_history.items():
            lenw = len(weights)
            # print(f"{key} weights (len={len(weights)}): {weights}")

        # print(lenw)
        if lenw > 7 and lenw%2!=0:
            logging.info(f"Weight SoftAdapt Loss {self.adapt_weight}")
        # Tính trọng số động với SoftAdapt
            self.adapt_weight = self.softadapt.get_component_weights(
                *[torch.tensor(self.weight_history[key], device='cpu') for key in self.weight_history],
                verbose=False
            )
            self.weight_history = {
                'conf': [],
        ...
        # Áp dụng trọng số
        weighted_losses = [loss * weight for loss, weight in zip(losses, self.adapt_weight)]

        total_loss = sum(weighted_losses)

        return total_loss, {name: weighted_loss for name, weighted_loss in zip(
            ['poly', 'lower', 'upper', 'cls_loss', 'line_iou', 'conf'], weighted_losses
        )}
