...
class CurvatureAwareFeatureBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3):
        super(CurvatureAwareFeatureBlock, self).__init__()
        
        # Convolution chính để xử lý đặc trưng
        self.conv_main = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=kernel_size // 2)

        # Tạo hai nhánh phụ để xử lý các độ cong khác nhau
        self.conv_curve1 = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, dilation=2, padding=2)
        self.conv_curve2 = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, dilation=3, padding=3)

        # Kết hợp thông tin từ nhánh chính và nhánh cong
        self.fusion = nn.Conv2d(out_channels * 3, out_channels, kernel_size=1)

        # Pooling để giảm nhiễu và tăng tính liên tục
        self.avg_pool = nn.AvgPool2d(kernel_size=2, stride=2)

    def spatial_transform(self, x, angle=15, warp_factor=0.1):
        """
        Áp dụng phép biến đổi không gian:
        - Xoay (rotate)
        - Warp (biến đổi không gian phi tuyến).
        """
        # Xoay (Rotate)
        rotated = TF.rotate(x, angle=angle)

        # Warp (thực hiện bằng cách dịch pixel theo một hướng)
        batch_size, channels, height, width = x.size()
        grid_x, grid_y = torch.meshgrid(torch.linspace(-1, 1, width), torch.linspace(-1, 1, height))
        grid = torch.stack((grid_x, grid_y), dim=-1).unsqueeze(0).repeat(batch_size, 1, 1, 1).to(x.device)

        # Thêm yếu tố biến đổi warp vào grid
        grid[:, :, :, 0] += warp_factor * torch.sin(grid[:, :, :, 1] * 3.14)
        grid[:, :, :, 1] += warp_factor * torch.cos(grid[:, :, :, 0] * 3.14)
    ...
    def forward(self, x):
        # Áp dụng phép biến đổi không gian
        rotated, warped = self.spatial_transform(x)

        # Nhánh chính
        main_features = self.conv_main(x)

        # Nhánh xử lý đường cong
        curve_features1 = self.conv_curve1(rotated)
        curve_features2 = self.conv_curve2(warped)

        # Đồng bộ kích thước giữa các nhánh
        max_height = max(main_features.size(2), curve_features1.size(2), curve_features2.size(2))
        max_width = max(main_features.size(3), curve_features1.size(3), curve_features2.size(3))

        main_features = F.interpolate(main_features, size=(max_height, max_width), mode="bilinear", align_corners=False)
        curve_features1 = F.interpolate(curve_features1, size=(max_height, max_width), mode="bilinear", align_corners=False)
        curve_features2 = F.interpolate(curve_features2, size=(max_height, max_width), mode="bilinear", align_corners=False)

        # Kết hợp các nhánh
        combined_features = torch.cat([main_features, curve_features1, curve_features2], dim=1)
        fused_features = self.fusion(combined_features)

        # Pooling để giảm nhiễu và kích thước
        output = self.avg_pool(fused_features)
        return output
...
class PolyRegression(nn.Module):
    def __init__(self,
                 ...
                 attention_heads=5,
                 use_flip_block=True,
                 use_curvature_block=True,
                 flip_axis=1):
        super(PolyRegression, self).__init__()

        self.use_flip_block = use_flip_block
        self.use_curvature_block = use_curvature_block
        ...
        self.papfn.register_forward_hook(hook_fn)  # Gắn hook vào PAPFN
        ...
        self.attention.register_forward_hook(hook_fn)  # Gắn hook vào Self-Attention
        ...
        if self.use_flip_block:
            ...
            self.flip_block.register_forward_hook(hook_fn)  # Gắn hook vào FeatureFlipBlock

        # Curvature-Aware Feature Block
        if self.use_curvature_block:
            self.curvature_block = CurvatureAwareFeatureBlock(in_channels=256, out_channels=256)
            self.curvature_block.register_forward_hook(hook_fn)  # Gắn hook vào CurvatureAwareFeatureBlock
    ...
    def _initialize_backbone(self, backbone, num_outputs, pretrained, extra_outputs):
        if 'mobilenet_v2' in backbone:
            ...
            # Gắn hook vào các stage trong backbone
            for stage in backbone_stages:
                stage.register_forward_hook(hook_fn)
            return backbone_stages
        else:
    ...
    def forward(self, x, epoch=None, **kwargs):
        ...
        # Apply Curvature-Aware Feature Block
        if self.use_curvature_block:
    ...
    def focal_loss(self, pred_confs, target_confs, alpha=0.2, gamma=2.0):
        ...
        :param target_confs: Target labels (0 hoặc 1).
