...
class SelfAttention(nn.Module):
    ...
    def forward(self, value, key, query, mask=None):
        ...
        output = self.norm(attention_output + query)  # Residual connection
...
class PolyRegression(nn.Module):
    def __init__(self,
        ...
        self.spp_output_size = self._compute_spp_output_size(spp_pool_sizes, feature_map_size=(12, 20))

        # Add a projection layer to reduce SPP output size
        self.spp_projection = nn.Linear(self.spp_output_size, num_outputs)

        # Output Layer
        self.output_layer = OutputLayer(nn.Linear(num_outputs, num_outputs), extra_outputs)
    ...
    def forward(self, x, epoch=None):
        ...
        # print(f"Shape after Self-Attention: {features.shape}")  # Debug
